...
clim = crop(clim, extent(c(boundary[2]-5, boundary[4]+5,boundary[1]-5, boundary[3]+5)))
extr = getextr(
  sp_bi,
  clim,
...
prec = monitors_clean$prcp;
year.prec = cbind(as.numeric(year), prec)
p = vector(); z = 1;
for(i in min(year.prec[,1]):(max(year.prec[,1])-1)){
  p[z] = sum(as.numeric(year.prec[which(year.prec[,1] == i), 2]), 
              na.rm=TRUE)
  z = z+1;
}
plot(density(p/10, na.rm=TRUE)) #distribution of precipitation in mm

ghcn.prec = mean(p/10, na.rm=TRUE)
...
abline(v=median(optim$origk[,1])/10, col='red')
